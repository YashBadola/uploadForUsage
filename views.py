#everything here is like a button that when pressed returns data from the web
...
from fibonacci_tmp import fibonacci_peak,global_peak_two_step
...
@gzip.gzip_page
def stream(request):
    return StreamingHttpResponse(scope.start_stream_object(), content_type="video/H264")
...
def autofocus(request,method,minZposition = 0,maxZposition=12000):#button
    '''
    apply
    fibonacci_peak,
    globalpeak
    ...
    '''
    minZposition = minZposition   #minimum position allowed in z-axis
    maxZposition = maxZposition   #maximum position allowed in z-axis
    hysteresis = 0                #hysterisis??
    tolerance = 5                 #minimum stepsize
    c_step = 200                  #coarse step
    f_step = 5                    #fine step
    if(method == "fibonacci"):  
        pos, complexity, loopCount = fibonacci_peak(minZposition,maxZposition,hysteresis,tolerance)
        print("complexity:", complexity)
        print("position:",pos)
        print("MovementCount:",loopCount)
    elif(method == "globalPeak"):
        pos, complexity, steps = global_peak_two_step(c_step,f_step,minZposition,maxZposition,hysteresis)
        print("complexity:", complexity)
        print("position:",pos)
        print("MovementCount:",steps)
    return HttpResponse(0)
